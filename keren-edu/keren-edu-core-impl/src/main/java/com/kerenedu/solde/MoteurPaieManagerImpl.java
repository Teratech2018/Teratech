    ...
    implements MoteurPaieManagerLocal, MoteurPaieManagerRemote  
{
	...
	public Double eval(RubriquePaie rubrique, Professeur salarie, PeriodePaie periode, Etablissement structure) {
		...
		if (rubrique.getMode().trim().equalsIgnoreCase("0")) {
			 result = evalCategorie(rubrique, salarie);
			 ...
			 result = evalPersonnel(rubrique, salarie);
	...
	private Double eval(RubriquePaie rubrique, Professeur salarie) {
		Double result=0.0;
		if (rubrique.getMode().trim().equalsIgnoreCase("0")) {
			 result = evalCategorie(rubrique, salarie);
		} else if (rubrique.getMode().trim().equalsIgnoreCase("1")) {
	...
	public BulletinPaie eval(BulletinPaie bulletin) {
		System.out.println("MoteurPaieManagerImpl.eval() début évaluation rubrique ");		  //Traitement des rubrique de type
...
//			   System.out.println("MoteurPaieManagerImpl.preparerPaie() début evaluation ==== ");
...
//       
	 ...
	 */
	  ...
	  private List<Professeur> creationBulletinPaiePeriode(PrepaSalaire entity){
  		...
  		//	Professeur prof= employdao.findByPrimaryKey("id", salarie.getId());
  		  Etablissement societe = this.getsociete();
  		  ...
  		  //Traitement des Elements variables(Prêt , avances,...)
             ...
             if(eltsvariables!=null&&!eltsvariables.isEmpty()){
 		    	...
 		    	//Rappel Salaires container = RestrictionsContainer.newInstance();
            ...
            if(salarie!=null&&salarie.getDateembauche()!=null){
                bulletin.setAnciennite(Double.parseDouble(""+DateHelper.numberOfMonth(salarie.getDateembauche(), bulletin.getPeriode().getDfin())));
            }//end if(contrat!=null){
	  ...
	  /**
		 ...
		 * @return
		 */
		private Double evalCategorie(RubriquePaie rubrique, Professeur salarie){
			Double valeur = 0.0;
...
//					System.out.println(MoteurPaieManagerImpl.class.toString()+".evalCategorieProf(Rubrique rubrique, Employe salarie) ========== forfait : "+forfait.getCategorie()+" === salarie : "+salarie.getFonction()+" == valeur :"+valeur);
		 ...
		 * @return
		 */
		private Double evalPersonnel(RubriquePaie rubrique, Professeur salarie){
			Double valeur = 0.0;
//			if(salarie.getCategorie()==null){
//				return valeur;
//			}//end if(salarie.getFonction()==null)
