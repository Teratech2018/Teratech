...
DROP VIEW e_zview_note_helper_old ;

CREATE VIEW e_zview_note_helper_old(ID,MAT_NOTE_ID,CLASSE_ID,EXAMEN_ID,MATIERE_ID,NOTE_ID,ELEVE_ID,NOTE,NOTE1,NOTE2,NOTE3,
...
Truncate((IFNULL(notemat(e.INSCRIPTION_ID ,l.mat_id,'0',e.annee_id),0)+IFNULL(notemat(e.INSCRIPTION_ID ,l.mat_id,'1',e.annee_id),0))/2,2),
Truncate((IFNULL(notemat(e.INSCRIPTION_ID ,l.mat_id,'2',e.annee_id),0)+IFNULL(notemat(e.INSCRIPTION_ID ,l.mat_id,'3',e.annee_id),0))/2,2),
Truncate((IFNULL(notemat(e.INSCRIPTION_ID ,l.mat_id,'4',e.annee_id),0)+IFNULL(notemat(e.INSCRIPTION_ID ,l.mat_id,'5',e.annee_id),0))/2,2),n.appreciation,
...
CREATE VIEW e_zview_helper AS select concat(`e`.`ETUDIANT_ID`,`x`.`id`,`m`.`CLASSE_ID`) AS `ID`,
`x`.`id` AS `EXAMEN_ID`,`e`.`ETUDIANT_ID` AS `ETUDIANT_ID`,`m`.`CLASSE_ID` AS `CLASSE_ID`,
ifnull((sum((`e`.`NOTE` * `c`.`COEF`)) / sum(`c`.`COEF`)),0) AS `MOY`,`e`.`ANNEE_ID` AS `ANNEE_ID`,`x`.`LIBELLE` AS `TYPE_EXAMEN` 
from (((`e_notedlt` `e` join `e_note_mat` `m`) join `e_coefmatdtl` `c`) join `e_examen` `x`) where ((`e`.`EL_NOTE_ID` = `m`.`id`) 
and (`m`.`MATIERE_ID` = `c`.`id`) and (`m`.`EXAMEN_ID` = `x`.`id`) and (`e`.`NOTE` <> 0)) group by `e`.`ETUDIANT_ID`,`x`.`id`,`e`.`ANNEE_ID`;



DROP VIEW e_zview_helper_bull ;
...
DROP VIEW e_zview_helper_ligne ;
...
--bulletin primaire
CREATE VIEW e_zview_bulletin_prim(ID,INS_ID,ELEVE_ID,CLASSE_ID,CYCLE_ID,ANNEE_ID,COEF_ID,MAT_ID,EXAMEN_ID,NOTE_ID,
MOY1,MOY2,MOY3,RANG1,RANG2,RANG3,RANK_MOY,RANG_MAT,
MOIS1,MOIS2,MOIS3,MOIS4,MOIS5,MOIS6,MOIS7,MOIS8,RGM1,RGM2,RGM3,RGM4,RGM5,RGM6,RGM7,RGM8,
MOY_GEN_CLS,MOY_PREMIER,MOY_DERNIER,NBRE_MOY,NBRE_ELVE,TX_REU,TOTAL_POINT,
ECART_TYPE,APP_MAT, APP,SANCTION,APP_EN,
DESIGNATION,EDITTITLE,LISTTITLE,MODULENAME,CREATEONFIELD,COMPAREID,SELECTED,DESABLECREATE,ACTIVATEFOLLOWER,ACTIVEFILELIEN,DESABLEDELETE,FOOTERSCRIPT,SERIAL,
DESABLEUPDATE,searchkeys,desabledatablock,ownermodule)
AS
...
IFNULL(moyeleseq(i.id,'6',i.annee_id), 0),IFNULL(moyeleseq(i.id,'7',i.annee_id), 0),IFNULL(moyeleseq(i.id,'8',i.annee_id), 0),
rankseqn(i.id ,'6',i.annee_id,i.classe_id),rankseqn(i.id ,'7',i.annee_id,i.classe_id),rankseqn(i.id ,'8',i.annee_id,i.classe_id),
rankmoyenne(i.id ,e.examen_id,i.annee_id,i.classe_id),rankmat(i.id ,e.matiere_id,e.examen_id,i.annee_id),
moymois(i.id ,'6',i.annee_id,"M1"),moymois(i.id ,'6',i.annee_id,"M2"),moymois(i.id ,'6',i.annee_id,"M3"),
moymois(i.id ,'7',i.annee_id,"M1"),moymois(i.id ,'7',i.annee_id,"M2"),moymois(i.id ,'7',i.annee_id,"M3"),
moymois(i.id ,'8',i.annee_id,"M1"),moymois(i.id ,'8',i.annee_id,"M2"),
rankmois(i.id ,'6',i.annee_id,i.classe_id,"M1"),rankmois(i.id ,'6',i.annee_id,i.classe_id,"M2"),rankmois(i.id ,'6',i.annee_id,i.classe_id,"M3"),
rankmois(i.id ,'7',i.annee_id,i.classe_id,"M2"),rankmois(i.id ,'7',i.annee_id,i.classe_id,"M2"),rankmois(i.id ,'7',i.annee_id,i.classe_id,"M2"),
rankmois(i.id ,'8',i.annee_id,i.classe_id,"M3"),rankmois(i.id ,'8',i.annee_id,i.classe_id,"M3"),
moygencls(i.classe_id,e.examen_id,i.annee_id),moypremiercls(i.classe_id,e.examen_id,i.annee_id),moydercls(i.classe_id,e.examen_id,i.annee_id),
nbremoycls(i.classe_id,e.examen_id,i.annee_id),nbreelevecls(i.classe_id,i.annee_id),TRUNCATE(((nbremoycls(i.classe_id,e.examen_id,i.annee_id) *100)/nbreelevecls(i.classe_id,i.annee_id)),3),
totalpointexamen(i.id,e.examen_id,i.annee_id),ecarttypemoy(i.classe_id,e.examen_id,i.annee_id),
n.APPRECIATION,appreciation(e.examen_id,i.id,i.annee_id),sanction(e.examen_id,i.id,i.annee_id),appreciationen(e.examen_id,i.id,i.annee_id),

"defualt","defualt","defualt","defualt",0,0,0,0,0,0,0,"defualt","defualt",0,"","",""
from e_inscription i, e_coefmatdtl mat,e_note_mat e, e_notedlt n, e_examen x
where i.id=n.etudiant_id
and e.id=n.el_note_id
and mat.id=e.matiere_id
and x.id=e.examen_id
and type_cycle_id=1
...
DELIMITER $$
CREATE FUNCTION `moyelehelperm`(param BIGINT(20),seq BIGINT(20),exercice varchar(255),mois varchar(255)) RETURNS decimal(38,2)
begin

    DECLARE valeur decimal(38,2);
	
	    if mois='M1'
       then 	SELECT sum(e.s_note_1*c.coef)/sum(c.coef) into valeur from  e_notedlt e , e_note_mat m , e_coefmatdtl c , e_examen x
					where e.el_note_id=m.id and m.matiere_id=c.id  and m.examen_id=x.id   and e.etudiant_id=param and e.annee_id=exercice and x.id=seq
					and e.s_note_1!=0;
					return valeur;
       elseif mois='M2'
       then  SELECT sum(e.s_note_2*c.coef)/sum(c.coef) into valeur from  e_notedlt e , e_note_mat m , e_coefmatdtl c , e_examen x
					where e.el_note_id=m.id and m.matiere_id=c.id  and m.examen_id=x.id   and e.etudiant_id=param and e.annee_id=exercice and x.id=seq
					and e.s_note_2!=0;
					return valeur;
        else
          SELECT sum(e.s_note_3*c.coef)/sum(c.coef) into valeur from  e_notedlt e , e_note_mat m , e_coefmatdtl c , e_examen x
					where e.el_note_id=m.id and m.matiere_id=c.id  and m.examen_id=x.id   and e.etudiant_id=param and e.annee_id=exercice and x.id=seq
					and e.s_note_3!=0;
					return valeur;
 end if ;
...
-- primaire fonction
DELIMITER $$
CREATE FUNCTION `moymois`(param BIGINT(20),seq varchar(255),exercice varchar(255),mois varchar(255)) RETURNS decimal(38,2)
begin
    DECLARE valeur decimal(38,2);
	    if mois='M1'
       then  SELECT sum(e.s_note_1*c.coef)/sum(c.coef) into valeur from  e_notedlt e , e_note_mat m , e_coefmatdtl c , e_examen x
...
DELIMITER $$
CREATE FUNCTION `rankmois`(eleve BIGINT(20),examen varchar(255), exercice varchar(255), classe BIGINT(20),mois varchar(255)) RETURNS decimal(38,2)
begin
    DECLARE valeur decimal(38,2);
	
	    if mois='M1'
       then 	SELECT FIND_IN_SET(moy1,(select GROUP_CONCAT(DISTINCT moy1 ORDER BY moy1 DESC) FROM e_zview_helper e where e.type_examen=examen  and annee_id=exercice and e.classe_id=classe )) into valeur from e_zview_helper e
				where e.type_examen=examen and e.etudiant_id=eleve and annee_id=exercice and e.classe_id=classe ;
				return valeur ;
       elseif mois='M2'
       then  SELECT FIND_IN_SET(moy2,(select GROUP_CONCAT(DISTINCT moy2 ORDER BY moy2 DESC) FROM e_zview_helper e where e.type_examen=examen  and annee_id=exercice and e.classe_id=classe )) into valeur from e_zview_helper e
				where e.type_examen=examen and e.etudiant_id=eleve and annee_id=exercice and e.classe_id=classe ;
				return valeur ;
        elseif mois='M3'
       then   SELECT FIND_IN_SET(moy3,(select GROUP_CONCAT(DISTINCT moy3 ORDER BY moy3 DESC) FROM e_zview_helper e where e.type_examen=examen  and annee_id=exercice and e.classe_id=classe )) into valeur from e_zview_helper e
				where e.type_examen=examen and e.etudiant_id=eleve and annee_id=exercice and e.classe_id=classe ;
				return valeur ;
	end if ;
...
-- new bulletin
SELECT concat(e.id,l.id),l.id, e.id, e.INSCRIPTION_ID,e.INS_ID, e.cls_id, e.examen_id, e.moyenne,
e.rang , notematbul(e.INSCRIPTION_ID ,l.mat_id,'0',e.annee_id), notematbul(e.INSCRIPTION_ID ,l.mat_id,'1',e.annee_id),
notematbul(e.INSCRIPTION_ID ,l.mat_id,'2',e.annee_id), notematbul(e.INSCRIPTION_ID ,l.mat_id,'3',e.annee_id),
notematbul(e.INSCRIPTION_ID ,l.mat_id,'4',e.annee_id), notematbul(e.INSCRIPTION_ID ,l.mat_id,'5',e.annee_id),
...
rankseqnbul(e.INSCRIPTION_ID ,'2',e.cls_id,e.annee_id),rankseqnbul(e.INSCRIPTION_ID ,'3',e.cls_id,e.annee_id),
rankseqnbul(e.INSCRIPTION_ID ,'4',e.cls_id,e.annee_id),rankseqnbul(e.INSCRIPTION_ID ,'5',e.cls_id,e.annee_id),
appreciationbull(e.moyenne),sanctionbull(e.moyenne),abscence(e.examen_id,e.INSCRIPTION_ID,e.annee_id),appreciationenbull(e.moyenne),
...
SELECT e.INSCRIPTION_ID, l.mat_id, l.notet1 ,FIND_IN_SET(l.notet1,(select GROUP_CONCAT( DISTINCT l.notet1 ORDER BY l.notet1 DESC)  FROM e_bul e, e_bul_lgn l
where e.id=l.lgn_bul_id and l.mat_id=97   and e.annee_id=2018 and e.cls_id=34 ) ) from e_bul e, e_bul_lgn l
where e.id=l.lgn_bul_id and l.mat_id=97  and e.annee_id=2018
