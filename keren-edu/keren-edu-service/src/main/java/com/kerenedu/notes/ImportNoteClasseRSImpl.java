...
public class ImportNoteClasseRSImpl extends AbstractGenericService<ImportNoteClasse, Long>
		implements ImportNoteClasseRS {
	// @Manager(application="kereneducation", name="ImportNoteManagerImpl",
	// interf=ImportNoteManagerRemote.class)
	...
	public MetaData getMetaData(HttpHeaders headers) {
		try {
			return MetaDataUtil.getMetaData(new ImportNoteClasse(), new HashMap<String, MetaData>(),
	...
	public ImportNoteClasse save(HttpHeaders headers, ImportNoteClasse entity) {
		...
		//forceupdate e_note_dlt
		managernote.updateforce(entity);
		return entity;
	}

	private List<NoteDetail> getNoteStudent(ImportData entity, ImportNoteClasse critere,
			ImportNoteClasseFile noteFile) {
			...
			if (exists) {
				...
				if (datas != null) {
					...
					if ((entities != null) && (!entities.isEmpty())) {
						...
						List<MatiereNote> results = managermat.filter(container.getPredicats(), null, new HashSet(), 0,
								-1);
...
//						} else {
//							// creer matiere
//							MatiereNote m = this.addMatiere(critere);
//							System.out.println("ImportNoteClasseRSImpl.getNoteStudent() matiere "+m.getId());
//							m.setNotelisttr(entities);
//							managermat.importNote(m);
//						}
					}
				}
			} else {
	...
	protected MatiereNote addMatiere(ImportNoteClasse critere){
		RestrictionsContainer container = RestrictionsContainer.newInstance();
		MatiereNote mat= new MatiereNote();
		 // creer la matiere 
		...
		for(CoefMatiereDetail mt : listMatieres ){
			...
			if(notebd==null||notebd.isEmpty()||notebd.size()==0){
	   	   			MatiereNote mtrt = new MatiereNote(mt,critere.getExamen(),eleves);
	   	   			mtrt.setId(-1);
	   	   	 mat = managerNote.save(mtrt);
			}
			
    }//fin if(mt.getMatiere().isState()){
  }//finfor(CoefMatiereDetail mt : listMatieres ){
		return mat;
	}
	protected List<NoteDetail> mapToJavaObjectNote(String classname, Map<Long, List<String>> data)
			...
			if (key > 0L) {
				...
				for (String fieldname : fieldNames) {
					...
					if (names6.length <= 1) {
						...
						if (field != null) {
							Object value = CommonTools.getEntity(getManager().getDao().getEntityManager(), names6[0],
									names6[1], (String) values.get(index), field);
	...
	protected void affectValue(NoteDetail object, Field field, String value)
